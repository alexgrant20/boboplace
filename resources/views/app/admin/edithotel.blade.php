          ...
          <div class="row">
            <div class="col">
              <div class="d-flex justify-content-center align-items-center" style="border-radius: 1em; height: 100%;">
                ...
                <label for="firstimg">
                  <img src="{{ $path }}" width="420px" height="350px" style="cursor: pointer;"
                    id="display-image">
                </label>
              </div>
            </div>
            ...
            <div class="col-7 mt-3">
              <div class="mb-3">
                ...
                <input type="text" class="form-control" value="{{ $hotel->name }}" class="text-black" id="hotelname"
                  name="name">
                @error('name')
                  <div>
                    {{ $message }}
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control" value="{{ $hotel->full_address }}" id="address"
                  name="full_address">
                @error('address')
                  <div>
                    {{ $message }}
                  ...
                  <div class="col-12">
                    @foreach ($facilities as $facility)
                      <div class="d-inline-flex align-items-center gap-2 me-3">
                        @php
                          $facilitiesId = $facilityHotel->pluck('facility_id')->toArray();
                        @endphp
                        @if (in_array($facility->id, $facilitiesId))
                          <input class="form-check-input mt-0" type="checkbox" id="facility-{{ $facility->id }}"
                            name="{{ $facility->id }}" value="1" checked>
                        @else
                          <input class="form-check-input mt-0" type="checkbox" id="facility-{{ $facility->id }}"
                            name="{{ $facility->id }}" value="1">
                        @endif
                        <label class="form-check-label text-white" for="facility-{{ $facility->id }}">
                          {{ $facility->name }} </label>
                      </div>
                    @endforeach
                  </div>
